...
短期内用户视图已可从 `user_follow_snapshots` 命中；没有用户快照时继续按 `follow_start_date` 从策略运行表或回放派生，并写回用户跟随表。
...
1. 把推荐和日计划生成改为后台定时预计算，前台优先读缓存。
2. 后台增加“策略运行矩阵”，能看到每个策略是否运行、进度、最新信号、收益。
3. 慢接口全部改为后台任务 + 进度查询 + 缓存读取。
4. 用户管理页补单用户周期详情和账户追溯。
