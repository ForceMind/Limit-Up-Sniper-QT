...
- v0.2.48 起，前台推荐和日计划新增后台预计算任务；缓存未命中时默认排队生成 `frontend_payload_cache`，前台请求返回 pending，不再长时间等待同步计算。
...
- 推荐和日计划已完成后台预计算第一版；覆盖率、回测等慢接口还没有全部任务化。
...
- v0.2.48 起，前台推荐和日计划缓存未命中时默认触发 `frontend_payload_precompute`，同步请求只返回轻量 pending 状态；后台可手动预计算，调度器也可按间隔触发。

### 6.3 下一步性能重点

- 推荐和日计划生成已改为后台定时预计算第一版，前台缓存未命中时默认排队生成。
...
继续推进 P3：覆盖率、回测等剩余慢接口任务化和预计算。

原因：

- 用户跟随周期、快照、持仓、成交和后台诊断已经有第一版闭环。
- 登录后推荐和日计划已默认从 HTTP 同步计算中剥离；覆盖率、部分回测和数据诊断仍需要继续任务化。
- 后台预计算可以继续降低 Nginx/Cloudflare 等待后端超时的概率。

建议下一次开发目标：

> 把覆盖率、回测详情和其它可能超过 5 秒的诊断接口继续改为后台任务 + 进度 + 缓存读取。
