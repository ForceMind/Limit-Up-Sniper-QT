...
- 策略运行结果已有每日信号、成交、持仓、账户快照和清算结果第一版落库；用户跟随账户已有快照、持仓、成交第一版落库。
- 用户跟随周期变更记录和后台用户账户诊断页还未完善。
...
- 补齐用户跟随周期变更记录，并在后台用户管理页展示当前账户诊断。
- 把推荐和日计划生成改为后台定时预计算。
...
优先继续 P2：用户跟随账户落库后的周期记录和后台诊断。

原因：

- 用户跟随快照、持仓和成交已经有第一版落库，但还缺少显式周期变更记录。
- 后台用户管理需要能定位用户当前资金、策略、跟随开始日、账户快照和最近成交。
- 这能把“策略结果”和“用户账户”彻底分开，后续再做推荐/日计划预计算更稳。

建议下一次开发目标：

> 实现用户跟随周期记录和后台账户诊断第一版：记录用户资金/策略切换周期，并在后台用户管理页展示当前跟随账户快照、持仓和最近成交来源。
