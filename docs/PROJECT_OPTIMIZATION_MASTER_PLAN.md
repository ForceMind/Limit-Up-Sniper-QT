...
- 策略独立运行：每个策略有自己的参数、信号、成交、持仓、账户快照、交割单、资金流水和训练轨迹。
...
- 策略运行结果已有每日信号、成交、持仓第一版落库；`strategy_runtime_snapshots` 仍主要承担前台账户短缓存，还未升级为正式每日账户快照。
- 用户跟随账户还没有独立落库，目前仍由策略结果或回放视图派生。
...
策略运行层
  每个策略每日生成信号、目标仓位、成交、持仓、账户快照、交割单
...
- 后台基准参数只用于诊断、默认参数和人工调参，不代表用户跟随账户。
- 策略库里的模型才是用户可选择的跟随对象。
...
- `strategy_runtime_settlements`：每个策略每天的资金流水和清算。
- `user_follow_snapshots`：用户跟随账户快照。
...
- 后台策略复盘按资金档预设和策略库模型批量写入每日信号、成交、持仓运行表。
...
- 将 `strategy_runtime_snapshots` 从短缓存继续升级为正式每日账户快照，补齐每日资金流水/清算。
- 把推荐和日计划生成改为后台定时预计算。
...
- 后续新增 `strategy_runtime_settlements`。
- 后续将现有 `strategy_runtime_snapshots` 从短缓存升级为每日快照。
...
- 新增用户跟随策略变更记录。
- 用户设置资金或切换策略时生成新的跟随周期。
- 后台用户管理页展示用户当前账户、跟随开始日、最近访问。
...
- 这是解决服务器慢、用户跟随后持仓不明显变化、策略收益看不清的关键。
- 当前缓存只是短期缓解，真正成熟必须把策略运行结果长期保存。
- 完成 P1 后，前台、后台、用户跟随、策略库都会更清晰。

建议下一次开发目标：

> 实现策略每日运行结果落库第一版：新增 `strategy_daily_signals`、`strategy_runtime_positions`、`strategy_runtime_trades`，让后台策略复盘任务把每个策略的每日信号、成交和持仓写入 SQLite，并让前台账户优先读取这些结果。
