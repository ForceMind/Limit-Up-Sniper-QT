...
- 分时模拟：使用必盈 5 分钟 K 线模拟更接近盘中的买入、卖出和估值。
...
backend/app/quant/biying_sync.py 必盈行情同步
...
```text
http://127.0.0.1:8000
后台入口用 `qt admin-path` 或 `bash qt.sh admin-path` 查看，例如 http://127.0.0.1:8000/admin-a1b2c3d4
```
...
- `BIYING_ENABLED`：是否启用必盈行情同步
- `BIYING_LICENSE_KEY`：必盈数据授权
...
如果部署后仍看到样例数据，通常是还没有在后台“配置与安全”里填写 DeepSeek、必盈、邮件等服务器本地配置，也没有触发真实数据同步。配置完成后进入“运维”，点击“系统启动”，系统会按顺序执行新闻抓取、AI 分析、日K补齐、龙虎榜同步、行情同步、交易循环和策略复盘。
...
```bash
cp .env.example .env
# 编辑 .env，填入 DeepSeek、必盈、SMTP 等真实配置
bash qt.sh install
```
